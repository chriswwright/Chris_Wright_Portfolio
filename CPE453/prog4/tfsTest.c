  ...
  if (tfs_mount (DEFAULT_DISK_NAME) < 0)	/* if mount fails */
    {
      ...
      int m_status = tfs_mount (DEFAULT_DISK_NAME);
      if(m_status < 0)   
  ...
  if (tfs_readByte (aFD, &readBuffer) < 0)
    {
      /* if readByte() fails, there was no afile, so we write to it */
        ...
        char* test_b = calloc(9, sizeof(char));
        test_b[0] = '1';
        test_b[2] = '2';
        test_b[8] = '3'; 
	...
	printf ("Successfully written to afile\n");
          time_t test = tfs_lastWriteTime(aFD);
          printf("Write Time: %s\n", ctime(&test));
          sleep(2);
          for(int i = 0; i < 256; i++){
	      tfs_writeByte (cFD, '!');
          } 
          tfs_writeFile (aFD, "a", 1);
	  int l_status = tfs_writeByte (cFD, '!');
          printf("l_status: %d", l_status);
          test = tfs_lastWriteTime(aFD);
          printf("Write Time: %s\n", ctime(&test));
     }
  else
    {
...
/* now bfile tests */
  ...
  if (tfs_readByte (bFD, &readBuffer) < 0)
    {
      int m_status = tfs_writeFile (bFD, bfileContent, bfileSize);
      if(m_status < 0)
	{
          printf("m_status: %d", m_status);
	  perror ("tfs_writeFile failed");
	}
      else
	printf ("Successfully written to bfile\n");
        tfs_seek(bFD, -1);
        m_status = tfs_writeByte (bFD, '!');
        if(m_status < 0){
            printf("test: %d\n", m_status);
        }
        m_status = tfs_writeByte (bFD, '!');
